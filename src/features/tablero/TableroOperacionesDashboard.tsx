...
export const TableroOperacionesDashboard = () => {
  ...
  const columnas = useMemo(() => {
    ...
    ops.forEach((o) => {
      const st = norm(o.status) || '(Sin status)';
      if (!porStatus.has(st)) porStatus.set(st, []);
      porStatus.get(st)!.push(o);
    });
    // Orden natural: por el número inicial del status ("1. Pre-Documentado" …)
    ...
    <div className="dashboard-container tb-contenedor">
      ...
      {cargando ? <p className="tb-cargando">⏳ Cargando operaciones…</p> : (
        <div className="tb-tablero">
          {columnas.map(([status, lista]) => (
            <div key={status} className="tb-columna" onDragOver={(e) => e.preventDefault()} onDrop={(e) => soltarEn(status, e)}>
