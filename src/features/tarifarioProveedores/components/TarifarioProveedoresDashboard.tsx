...
export function TarifarioProveedoresDashboard() {
  ...
  const cambiarMontoVigenteLinea = async (r: Doc, idx: number, monto: number) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'tarifario_proveedores', r.id), { tarifas });
      const det = detalleDeLinea(r, lineas[idx]);
      if (det) { try { await updateDoc(doc(db, 'convenios_proveedores_detalles', String(det.id)), { tarifa: monto }); } catch { /* lo cubre el motor v1.3 */ } }
      await registrarLog('Tarifario Proveedores', 'Edición', `Cambió el monto vigente de la tarifa "${lineas[idx]?.descripcion || ''}" del pre convenio de "${r.proveedorNombre}" a ${fmtMoney(monto)}.`);
    } catch (e) {
