...
export function TarifarioProveedoresDashboard() {
      ...
      const payload = {
        ...
        tarifas: elegidas.map((t) => {
          ...
          return {
            ...
            cotizadoEn: monedaTarifa[t.id] || monedaProveedor || 'USD',
            status: 'Pendiente',
          };
        }),
        status: 'Pendiente',
  ...
  const construirHTMLTarifario = (r: Doc): string => {
    ...
    return `<!DOCTYPE html><html lang="es"><head><meta charset="utf-8">
      <title>TARIFAS_${esc(String(r.proveedorNombre || 'CLIENTE').toUpperCase().replace(/[^A-Z0-9]+/g, '_'))}_${esc(String(r.fecha || '').slice(0, 4))}</title>
        ...
        {registros.length === 0 ? (
          ...
          <div className="tc-marco">
            <table className="tc-tabla">
              <thead>
                <tr><th>ACCIONES</th><th>CONSECUTIVO</th><th>FECHA</th><th>PROVEEDOR</th><th>MONEDA</th><th>CRÉDITO</th><th>TARIFAS</th><th>STATUS</th></tr>{/* ✅ V00205: acciones primero */}
              </thead>
              <tbody>
                {registros.map((r) => (
                  ...
                  <tr key={r.id} className="tc-fila-click" onClick={() => setDetalleId(r.id)}>
                    ...
                    <td>{r.fecha || '—'}</td>
                    <td className="tc-td-cliente">{r.proveedorNombre || '—'}</td>
          ...
          <div className="modal-overlay tc-overlay" onClick={() => setDetalleId('')}>
            <div className="tc-modal tc-modal-detalle" onClick={(e) => e.stopPropagation()}>
              <div className="tc-modal-encabezado">
                <div>
                  <h3 className="tc-modal-titulo">Detalle del Pre Convenio — <span className="tc-td-cliente">{r.proveedorNombre || '—'}</span></h3>
                        ...
                        <tr key={t.id} className={marcada ? 'tc-fila-marcada' : ''} onClick={() => toggleTarifa(t)}>
                          ...
                          <td className="tc-td-num" onClick={(e) => e.stopPropagation()}>
                            <select
                              ...
                              value={monedaTarifa[t.id] || monedaProveedor || 'USD'}
