...
export const FormularioConvenioCliente = ({ estado, initialData, registrosExistentes, onClose, onMinimize, onRestore }: FormProps) => {
  ...
  const actualizarMonedaConvenio = async () => {
    const entId = String(formData.clienteId || '');
    ...
    try {
      ...
      if (initialData?.id) {
        await updateDoc(doc(db, 'convenios_clientes', String(initialData.id)), { monedaId: monId, monedaNombre: monNom });
  ...
  const guardarDetallesAhora = async () => {
    ...
    try {
      ...
      for (const delId of detallesEliminados) {
        try { await setDoc(doc(db, 'convenios_clientes_detalles', delId), { _eliminado: true }, { merge: true }); } catch { /* se borra con el maestro */ }
      }
          ...
          <div className={`fcc-cuerpo${estado === 'minimizado' ? ' oculto' : ''}`}>
            <form onSubmit={handleSubmit}>
              <div className="form-grid fcc-x18">
                ...
                <button type="button" className="fcc-btn-act-moneda" disabled={actualizandoMonedaConv} onClick={actualizarMonedaConvenio} title="Vuelve a leer la moneda de la empresa en la tabla Empresas y la aplica a este convenio">{actualizandoMonedaConv ? '⏳…' : '⟳ Actualizar moneda'}</button>
...
<small className="fcc-hint-guardar">La moneda se guarda en Firestore al instante al elegirla. Tarifas y tipos: presiona <b>💾 Guardar detalles</b> (o Guardar Convenio Maestro).</small>
