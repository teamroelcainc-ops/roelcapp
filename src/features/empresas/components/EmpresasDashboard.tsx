      ...
      <div className="ed-x7">
        ...
        <div className="ed-x9">
          ...
          <div className="ed-x17">
            ...
            {/* ✅ V00342: escribe la razón social actual de TODAS las empresas
                en operaciones, facturación y pagos (desnormalizado). */}
            <button
              type="button"
              className="btn btn-outline ed-razon-btn"
              title="Escribir la razón social actual de todas las empresas en operaciones, facturación y pagos"
              onClick={async () => {
                if (!window.confirm('Se escribirá la RAZÓN SOCIAL actual de todas las empresas en operaciones, facturación y pagos. ¿Continuar?')) return;
                try { const msg = await (await import('../services/sincronizarRazonSocial')).sincronizarRazonSocial(); alert(msg); }
                catch (e) { alert(`No se pudo sincronizar: ${(e as Error)?.message || e}`); }
              }}
            >🏷 Razón social</button>
            <button 
