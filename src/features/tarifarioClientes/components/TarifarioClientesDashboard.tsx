...
export function TarifarioClientesDashboard() {
  ...
  const sincronizarConvenios = async (r: Doc) => {
    ...
    try {
      ...
      const snapDet = convId ? await getDocs(query(collection(db, 'convenios_clientes_detalles'), where('convenioId', '==', convId))) : null;
      ...
      for (let i = 0; i < tarifas.length; i++) {
        ...
        if (det) {
          ...
          usados.add(cc);
          try { await updateDoc(doc(db, 'convenios_clientes_detalles', String(det.id)), { status: String(t.status || 'Aprobado'), tarifarioId: String(r.id) }); } catch { /* motor */ }
          ligadas += 1;
          continue;
        }
      ...
      if (ligadas + creadas > 0) {
        await updateDoc(doc(db, 'tarifario_clientes', r.id), { tarifas });
        await registrarLog('Tarifario Clientes', 'Edición', `Sincronizó convenios del pre convenio de "${razonSocialDe(r)}": ${ligadas} línea(s) ligada(s) y ${creadas} creada(s).`);
