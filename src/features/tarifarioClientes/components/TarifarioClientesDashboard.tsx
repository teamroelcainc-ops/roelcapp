...
export function TarifarioClientesDashboard() {
  ...
  const sincronizarConvenios = async (r: Doc) => {
    ...
    try {
      ...
      const snapDet = await getDocs(query(collection(db, 'convenios_clientes_detalles'), where('convenioId', '==', convId)));
      ...
      if (ligadas + creadas + reparadas > 0) {
        await updateDoc(doc(db, 'tarifario_clientes', r.id), { tarifas });
        await registrarLog('Tarifario Clientes', 'Edición', `Sincronizó convenios del pre convenio de "${razonSocialDe(r)}": ${ligadas} ligada(s), ${reparadas} duplicado(s) reparado(s), ${creadas} creada(s).`);
