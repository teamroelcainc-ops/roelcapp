...
export function TarifarioClientesDashboard() {
  ...
  const alElegirDocFirmado = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const destino = storageRef(storage, `tarifarios_firmados/tarifario_clientes/${r.id}/${Date.now()}_${limpio}`);
      ...
      await registrarLog('Tarifario Clientes', 'Edición', `Subió el tarifario firmado de "${r.clienteNombre}" (${r.consecutivo || r.id}): ${archivo.name}.`);
