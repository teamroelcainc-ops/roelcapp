...
export function TarifarioClientesDashboard() {
  ...
  useBusquedaGlobal((t) => setBusquedaLista(t), 'los tarifarios de clientes'); // ✅ V00263: buscador global del topbar
  ...
  useEffect(() => {
    const f = filtrosDeUrl<{ busquedaLista?: string; filtroEntidad?: string; filtroStatus?: string; filtroMoneda?: string; ordenCol?: { col: ColOrdenLista; asc: boolean } | null }>('tarifarioClientes');
    if (!f) return;
  ...
  const guardarPreConvenio = async () => {
    ...
    try {
      ...
      if (editandoId) {
        ...
        if (docNuevoFile) { try { await subirDocFirmadoA(editandoId, docNuevoFile, String((clienteSel)?.nombre || '')); } catch (eDoc) { console.error(eDoc); alert('El tarifario se guardó, pero el documento firmado no se pudo subir. Súbelo desde la fila (📎).'); } } // ✅ V00274
      } else {
        ...
        if (docNuevoFile) { try { await subirDocFirmadoA(consecTar, docNuevoFile, String((clienteSel)?.nombre || '')); } catch (eDoc) { console.error(eDoc); alert('El tarifario se creó, pero el documento firmado no se pudo subir. Súbelo desde la fila (📎).'); } } // ✅ V00274
      }
      setModalAbierto(false);
      setCapturaAbierta(false);
      limpiarCaptura();
  ...
  const subirDocFirmadoA = async (id: string, archivo: File, nombreEntidad: string) => {
    ...
    const destino = storageRef(storage, `tarifarios_firmados/tarifario_clientes/${id}/${Date.now()}_${limpio}`);
    await uploadBytes(destino, archivo, archivo.type ? { contentType: archivo.type } : undefined);
    const url = await getDownloadURL(destino);
    await updateDoc(doc(db, 'tarifario_clientes', id), {
      docFirmadoUrl: url,
  ...
  const alElegirDocFirmado = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const url = await subirDocFirmadoA(String(r.id), archivo, String(r.clienteNombre || ''));
    ...
    try {
      ...
      if (!convenioId) {
        ...
        for (const t of (Array.isArray(r.tarifas) ? r.tarifas : [])) {
          ...
          if (!cc) continue;
          try { await updateDoc(doc(db, 'convenios_clientes_detalles', cc), { status: 'Aprobado', tarifarioId: String(r.id) }); } catch { /* detalle inexistente: lo cubre el motor */ }
        }
      }
    } catch (e) {
  ...
  const guardarConfigOblig = async () => {
    ...
    try {
      ...
      await registrarLog('Tarifario Clientes', 'Edición', `Cambió los campos obligatorios del formulario de tarifas (aplica a todos los usuarios).`);
  ...
  const sincronizarConvenios = async (r: Doc, opciones?: { silencioso?: boolean }) => {
    ...
    try {
      ...
      const guardarTarifas = async () => { await updateDoc(doc(db, 'tarifario_clientes', r.id), { tarifas }); };
      for (let i = 0; i < tarifas.length; i++) {
        const t = tarifas[i];
        ...
        if (det) {
          ...
          await updateDoc(doc(db, 'convenios_clientes_detalles', String(det.id)), { status: String(t.status || 'Aprobado'), tarifa: Number(t.tarifa) || 0, tarifarioId: String(r.id) });
          ligadas += 1;
          continue;
        }
        ...
        if (residuo) {
          ...
          usados.add(cc);
          await guardarTarifas();
          await updateDoc(doc(db, 'convenios_clientes_detalles', String(residuo.id)), { status: String(t.status || 'Aprobado'), tarifa: Number(t.tarifa) || 0, tarifarioId: String(r.id) });
          ligadas += 1;
          continue;
        }
        const [cc] = await reservarConsecutivosDetalle(1);
    ...
    <div className="tc-contenedor">
      ...
      <div className="tc-lista">
        ...
        <div className="tc-filtros">
          ...
          <a className="tc-btn-pestana" href={urlVerEnPestana('tarifarioClientes', { busquedaLista, filtroEntidad, filtroStatus, filtroMoneda, ordenCol })} target="_blank" rel="noopener noreferrer" title="Abrir esta vista en una pestaña nueva">↗</a>
            ...
            <table className="tc-tabla">
              <thead>
                <tr>{/* ✅ V00309: clic en la columna = ordenar (↑, ↓, tercer clic regresa al orden original) */}
                  ...
                  <th className="tc-th-orden" title="Ordenar por nombre" onClick={() => clicOrden('entidad')}>CLIENTE{flechaOrden('entidad')}</th>
